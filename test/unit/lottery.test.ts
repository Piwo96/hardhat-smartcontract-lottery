    ...
    : describe("Lottery", function () {
          ...
          describe("fulFilRandomWords", function(){
            ...
            it("Can only be called after request has been made", async function(){
                await expect(vrfMock.fulfillRandomWords(0, lottery.address)).to.be.revertedWith("nonexistent request");
                await expect(vrfMock.fulfillRandomWords(1, lottery.address)).to.be.revertedWith("nonexistent request");
            });
            ...
            it("Picks a winner, rests the lottery, and sends money", async function(){
                ...
                await new Promise<void>(async (resolve, reject) => {
                    ...
                    await vrfMock.fulfillRandomWords(txReceipt.events![1].args!.requestId, lottery.address);
                });
            });
      });
    });
